...
import mobile_pagefactory.ProductListing.FiltersPF.ResetFilterAPF;
...
public class ResetFilter {
    ...
    public void setup(@Optional("url") String env, @Optional("android") String browsername) throws IOException, InterruptedException {
        ...
        resetFilter = new ResetFilterAPF(driver);
...
//        Cart.clickOnVerifyNow();
    ...
    public void FilterByPrice() throws InterruptedException, FileNotFoundException {
        ...
        resetFilter.ClickOnFilter();
        getTest().log(Status.PASS, "Clicking on filter option");
        resetFilter.ClickOnPrice();
        getTest().log(Status.PASS, "Clicking on price");
        resetFilter.SelectPrice();
        getTest().log(Status.PASS, "Select price");
        resetFilter.ClickOnReset();
        getTest().log(Status.PASS, "Clicking on reset");
        resetFilter.SelectDifferentRange();
        getTest().log(Status.PASS, "Select different price range");
        resetFilter.Apply();
        getTest().log(Status.PASS, "Clicking on apply");
        resetFilter.ExitFromFilter();
